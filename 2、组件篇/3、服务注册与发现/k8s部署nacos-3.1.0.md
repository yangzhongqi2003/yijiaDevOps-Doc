...
```shell
kubectl exec -it mysql-0 -n nacos -- mysql -u root -pAdmin@2026 -e "alter user 'nacos'@'%' identified with mysql_native_password by 'nacos@2026';"

kubectl exec -it mysql-0 -n nacos -- mysql -u root -pAdmin@2026 -e "flush privileges;"
```

```shell
# sql地址
https://github.com/alibaba/nacos/blob/3.1.0/config/src/main/resources/META-INF/mysql-schema.sql

cd ~/nacos-yml && wget https://github.com/alibaba/nacos/raw/refs/tags/3.1.0/config/src/main/resources/META-INF/mysql-schema.sql

kubectl exec -it mysql-0 -n nacos -- mysql -u root -pAdmin@2026 nacos < mysql-schema.sql

kubectl exec -it mysql-0 -n nacos -- mysql -u root -pAdmin@2026 -e "use nacos;show tables;"
```

```shell
cat > ~/nacos-yml/nacos.yml << 'EOF'
apiVersion: v1
kind: Service
metadata:
  name: nacos-headless
        ...
        - name: k8snacos
          ...
          env:
            ...
            - name: NACOS_AUTH_TOKEN
              value: "nXTBjv3qHrbKoAuUPvCHd+PJGxyDBjeEvvxzl+v28xE="
            - name: NACOS_AUTH_IDENTITY_KEY
              value: "jLmmL0nXIO7HIhwjT8APwfknuzw442Wzx7Dj5gzEGLw="
            - name: NACOS_AUTH_IDENTITY_VALUE
              value: "QUpqJ15rKjI4q9m1xZKrpFGXHS2ACQQPWWo2z6nIWdc="
            - name: NACOS_SERVERS
...
```shell
kubectl apply -f ~/nacos-yml/nacos.yml
```
